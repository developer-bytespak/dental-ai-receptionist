...
import { useDemoState, type Scope } from "@/app/hooks/useDemoState";
...
export default function Desk({ scope, account, canReset }: DeskProps) {
  ...
  const { state, clear } = useDemoState(locationId, dayOffset, scope);

  /**
   * Clock dependent text differs between the server render and the browser,
   * so it is held back until the component has mounted.
   */
  ...
  const call = useRetellCall(locationName, scope);
      ...
      <aside className="sidebar" aria-label="Sections">
        <div className="side-brand">
          ...
          <div className="brand-text">
            ...
            <span className="brand-tagline">{scope === "demo" ? "AI front desk, demo" : "AI front desk"}</span>
          </div>
        </div>
        ...
        <div className="side-foot">
          ...
          {canReset ? (
            <button type="button" className="side-reset" onClick={onReset} disabled={resetting} title="Reset workspace">
              ...
              <span className="side-reset-hint">{scope === "demo" ? "Clears the board and the log" : "Removes every call and booking"}</span>
            </button>
          ) : null}
        ...
        <header className="topbar">
          <div className="greet">
            <h1 className="greet-title">
              {mounted ? greetingForHour(new Date().getHours()) : "Welcome"}, {practiceName.split(" ")[0]}
            </h1>
