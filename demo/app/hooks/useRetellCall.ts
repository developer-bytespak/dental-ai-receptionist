 ...
 * instant the agent reaches for the schedule, a beat before the server poll
 * confirms it.
 *
...
export interface ToolSignal {
  toolCallId: string;
  name: string;
  /** Undefined until the result comes back. */
  successful?: boolean;
}
...
export function useRetellCall(locationName: string, scope: "demo" | "app" = "demo"): UseRetellCall {
  ...
  useEffect(() => {
    ...
    fetch(`/api/config?scope=${scope}`, { cache: "no-store" })
      ...
      .catch(() => {
        /* The screen is still useful without it, the call button explains. */
      });
    return () => {
      alive = false;
    };
  ...
  const start = useCallback(async () => {
    ...
    const publicKey = config?.retell.publicKey ?? "";
    ...
    try {
      ...
      const session = client.createWebCall({
        ...
        retell_llm_dynamic_variables: {
          practice_name: config?.practice.name ?? "",
          location: locationRef.current,
          callback_number: config?.practice.callbackNumber ?? "",
        },
      ...
      const message =
        err instanceof Error ? err.message : "The call could not be connected.";
  ...
  }, [applyTranscript, config, stopRinging]);
  ...
  const end = useCallback(async () => {
    ...
    try {
      ...
      /* Already gone, the end hook has told us everything we need. */
    }
    sessionRef.current = null;
    setAgentTalking(false);
    setPhase("ended");
  ...
  // Never leave a live microphone behind on a hot reload or a route change.
  useEffect(() => {
    return () => {
      const session = sessionRef.current;
      sessionRef.current = null;
